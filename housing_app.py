...
def authenticate(credentials, username, password):
    if username in credentials["users"]:
        if credentials["users"][username]["password"] == password:
            return True
    return False

        
def housing_price_app():
...
# Main function to handle login and app display
def main():
    ...
    if not st.session_state["is_logged_in"]:
        ...
        if st.button("Login"):
            if authenticate(credentials, username, password):
                ...
                st.error("Incorrect username/password. Please try again.")
    else:
